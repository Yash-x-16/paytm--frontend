...
const sizeStyle = {
    ...
    "xl" :"py-3 px-4 w-full max-w-full text-lg  rounded-md flex text-center  m-2 justify-center",
    "2xl" :""
} 
...
export const BGbutton =(props:ButtonProp)=>{
 ...
 {props.onStart?<div className="  py-1  text-white">{props.onStart}</div>:null} <span className="text-center pl-2">{props.text}</span>{props.onEnd?<div className="pl-2 py-1">{props.onEnd}</div>:null}</button>
}